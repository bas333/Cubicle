Meteor.methods({
  ...
  'product.addcart':function(userid,product){
    console.log(userid);
    ...
    // }
    console.log(newcart);
    AllUsers.update({owner:userid},{$set:{cart:newcart}});
    console.log(user.cart);
  },
  'product.removecart':function(userid,product){
    var user=AllUsers.findOne({owner:userid});
    var newcart=user.cart;
  ...
  'chat.insert'(buyerid,product){
    ...
    Chat.insert(chat);
    const newchat=Chat.findOne({users_id:[sellerid,buyerid]});
 ...
 'reply.insert':function(reply){
   Reply.insert(reply);
   console.log(reply);
 },
 'opinion.insert':function(op){
   Opinion.insert(op);
 },
 'reply.delete':function(id){
   var post=Reply.findOne({_id:id});
   Reply.remove(post);
 }
})
